...
class Ngram(BaseCallback):
    ...
    Example:
        ...
        >>> preprocessor = mz.preprocessors.BasicPreprocessor(
                with_ngram=True, ngram_size=3)
        >>> preprocessor.fit_transform(data)
        >>> callback = Ngram(preproecssor=preprocessor, mode='index')
    ...
    def on_batch_unpacked(self, x, y):
        ...
        x['ngram_left'] = []
        x['ngram_right'] = []
        for row in x['text_left']:
            for term in row:
                x['ngram_left'].append(self._word_to_ngram[term])
        for row in x['text_right']:
            for term in row:
                x['ngram_right'].append(self._word_to_ngram[term])
